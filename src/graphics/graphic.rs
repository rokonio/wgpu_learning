...
pub struct GraphicBundle {
    ...
    pub uniform_bundle: UniformBundle,
}
...
impl GraphicBundle {
    pub async fn new(window: &Window, vertices: &[Vertex], indices: &[u16]) -> Self {
        ...
        let uniform_bundle = UniformBundle::new(&window_bundle, &camera);

        let pipeline_bundle = RenderPipelineBundle::new(
            &window_bundle,
            &[
                &texture_bundle.bind_group_layout,
                &uniform_bundle.uniform_bind_group_layout,
            ],
    ...
    pub fn render(&mut self) -> Result<(), wgpu::SwapChainError> {
        ...
        render_pass.set_bind_group(1, &self.uniform_bundle.uniform_bind_group, &[]);
    ...
    pub fn update(&mut self, camera: &Camera, _dt: time::Duration) {
